<div>
    @if ($submitted)
        <div class="mb-6 flex items-start gap-3 rounded-2xl bg-emerald-50 px-5 py-4 ring-1 ring-emerald-200">
            <svg class="mt-0.5 h-5 w-5 shrink-0 text-emerald-500" fill="none" viewBox="0 0 24 24" stroke-width="2"
                stroke="currentColor">
            ...
            <div>
                ...
                <p class="mt-0.5 text-xs text-emerald-700">Tu solicitud ha quedado registrada en estado
                    <strong>pendiente</strong>. El responsable de tu departamento será notificado para revisarla.</p>
            </div>
    ...
    <div class="grid gap-6 lg:grid-cols-[1fr_2fr]">
        <div class="flex flex-col gap-5">
            <div class="rounded-2xl bg-white p-5 ring-1 ring-slate-200 shadow-sm">
                ...
                <p class="mt-2 text-4xl font-semibold tabular-nums text-slate-900">{{ $this->remainingVacationDays }}
                </p>
                <p class="mt-1 text-xs text-slate-500">de {{ $vacationDays }} días anuales</p>
                @if ($requestType === 'vacation' && $this->requestedDays > 0)
                    <div class="mt-4 border-t border-slate-100 pt-4">
                        <div class="flex items-center justify-between text-xs text-slate-500">
                            <span>Días solicitados</span>
                            <span
                                class="font-semibold {{ $this->requestedDays > $this->remainingVacationDays ? 'text-red-600' : 'text-slate-900' }}">{{ $this->requestedDays }}</span>
                        </div>
                        <div class="mt-1.5 flex items-center justify-between text-xs text-slate-500">
                            ...
                            <span
                                class="font-semibold {{ $balance < 0 ? 'text-red-600' : 'text-emerald-600' }}">{{ $balance }}</span>
                        </div>
                        @if ($this->requestedDays > $this->remainingVacationDays)
                            <p class="mt-3 text-xs font-medium text-red-600">No tienes suficientes días disponibles.</p>
                        @endif
                    </div>
                @endif
            </div>
            ...
            <div class="rounded-2xl bg-white p-5 ring-1 ring-slate-200 shadow-sm">
                ...
                <p class="mt-0.5 text-xs text-slate-500">Rellena el formulario y envía tu solicitud.</p>
                <form wire:submit="submit" class="mt-5 flex flex-col gap-4">
                    <div>
                        <label class="block text-xs font-semibold text-slate-700">Tipo de solicitud <span
                                class="text-red-500">*</span></label>
                        <select wire:model.live="requestType"
                            class="mt-1.5 w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-900 outline-none transition focus:border-amber-400 focus:ring-2 focus:ring-amber-400/20">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="block text-xs font-semibold text-slate-700">Fecha inicio <span
                                    class="text-red-500">*</span></label>
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-slate-700">Fecha fin <span
                                    class="text-xs font-normal text-slate-400">(inclusive)</span> <span
                                    class="text-red-500">*</span></label>
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-slate-700">Comentario <span
                                class="text-xs font-normal text-slate-400">(opcional)</span></label>
        ...
        <div class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
            ...
            @if ($leaveRequests->isEmpty())
                <div class="px-5 py-12 text-center">
                    <p class="text-sm text-slate-500">Todavía no has enviado ninguna solicitud.</p>
                </div>
            @else
                <div class="overflow-x-auto">
                    <table class="w-full text-sm">
                        <thead>
                            <tr
                                class="border-b border-slate-100 text-xs font-semibold uppercase tracking-wider text-slate-400">
                            ...
                            @foreach ($leaveRequests as $request)
                                <tr class="transition hover:bg-slate-50/60">
                                    <td class="px-5 py-4 font-medium text-slate-900">
                                        {{ $request->request_type->label() }}</td>
                                    <td class="px-5 py-4 text-center tabular-nums text-slate-600">
                                        {{ $request->start_date->translatedFormat('d M Y') }}</td>
                                    <td class="px-5 py-4 text-center tabular-nums text-slate-600">
                                        {{ $request->end_date->translatedFormat('d M Y') }}</td>
                                    <td class="px-5 py-4 text-center">
                                        @php($status = $request->status)
                                        @if ($status === \App\Enums\LeaveRequestStatus::Approved)
                                            <span
                                                class="inline-flex rounded-full bg-emerald-50 px-2.5 py-0.5 text-xs font-semibold text-emerald-700 ring-1 ring-emerald-200">Aprobada</span>
                                        @elseif ($status === \App\Enums\LeaveRequestStatus::Rejected)
                                            <span
                                                class="inline-flex rounded-full bg-red-50 px-2.5 py-0.5 text-xs font-semibold text-red-700 ring-1 ring-red-200">Rechazada</span>
                                        @else
                                            <span
                                                class="inline-flex rounded-full bg-amber-50 px-2.5 py-0.5 text-xs font-semibold text-amber-700 ring-1 ring-amber-200">Pendiente</span>
                                        @endif
                                    </td>
                                    ...
                                    <td class="px-5 py-4 text-center text-xs tabular-nums text-slate-500">
                                        {{ $request->created_at->translatedFormat('d M Y') }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
