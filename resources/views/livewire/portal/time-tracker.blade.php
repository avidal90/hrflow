<div>
    ...
    @if ($activeEntry)
        <div wire:key="tracker-active" x-data="{
            baseElapsed: {{ $elapsedSeconds }},
            baseClientTs: null,
            elapsed: {{ $elapsedSeconds }},
            interval: null,
            init() {
                this.baseClientTs = Math.floor(Date.now() / 1000)
                this.elapsed = this.baseElapsed
                this.interval = setInterval(() => {
                    const nowTs = Math.floor(Date.now() / 1000)
                    this.elapsed = Math.max(0, this.baseElapsed + (nowTs - this.baseClientTs))
                }, 1000)
            },
            destroy() {
                if (this.interval) {
            ...
            <div class="flex flex-col items-start justify-between gap-6 p-6 sm:flex-row sm:items-center">
                <div>
                    ...
                    <p class="mt-3 font-mono text-5xl font-semibold tabular-nums tracking-tight" x-text="formatted">
                        {{ $elapsedLabel }}</p>
                    <p class="mt-2 text-sm text-slate-400">
                        Entrada registrada a las <span
                            class="font-medium text-white">{{ substr((string) $activeEntry->check_in_time, 0, 5) }}</span>
                    </p>
                </div>

                <button wire:click="stopTracking" wire:loading.attr="disabled"
    ...
    @else
        @if ($todayOffReason)
            <div wire:key="tracker-offday"
                class="mb-6 overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
        ...
        @else
            <div wire:key="tracker-inactive"
                class="mb-6 overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
                <div class="flex flex-col items-start justify-between gap-6 p-6 sm:flex-row sm:items-center">
                    <div>
                        <div class="flex items-center gap-2">
                            <span class="h-2 w-2 rounded-full bg-slate-300"></span>
                            <p class="text-xs font-semibold uppercase tracking-widest text-slate-400">Sin jornada activa
                            </p>
                        </div>
                ...
                <div class="grid gap-3 sm:grid-cols-2 lg:min-w-[22rem]">
                    <div class="rounded-xl bg-slate-50 px-4 py-3 ring-1 ring-slate-200/80">
                        ...
                        <p class="mt-1 text-lg font-semibold tabular-nums text-slate-900">
                            {{ $todayShiftSummary['workedLabel'] }}</p>
                    </div>
                    <div class="rounded-xl bg-amber-50 px-4 py-3 ring-1 ring-amber-200/80">
                        ...
                        <p class="mt-1 text-lg font-semibold tabular-nums text-amber-900">
                            {{ $todayShiftSummary['remainingLabel'] }}</p>
                    </div>
                </div>
            </div>
        </div>
