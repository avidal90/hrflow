    ...
    <div class="flex min-h-[calc(100vh-4rem)] items-center justify-center bg-slate-50 px-4 py-12 sm:px-6 lg:px-8">
        <div class="w-full max-w-md">
            <div class="mb-8 text-center">
                <div
                    class="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-amber-500 text-xl font-bold text-white shadow-lg shadow-amber-200">
                    HR
                </div>
                ...
                <form class="flex flex-col gap-5" method="POST" action="{{ $form_action }}">
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <input id="email" type="email" name="email" value="{{ old('email') }}" required
                            autofocus autocomplete="username" placeholder="tu@empresa.com"
                    ...
                    <button type="submit"
                        class="rounded-xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white transition hover:bg-slate-800 active:bg-slate-950">
                        Entrar
                    </button>
                </form>
            </div>
            ...
            <p class="mt-8 text-center text-xs text-slate-500">
                {{ $footer_prefix }}
                <a href="{{ $footer_link_url }}"
                    class="font-semibold text-slate-700 hover:text-slate-900">{{ $footer_link_text }}</a>.
            </p>
        </div>
    </div>
</x-layouts.public>
