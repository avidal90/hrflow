...
<x-layouts.portal :portal-user="$portalUser" :title="'Dashboard | ' . (tenant()?->name ?? 'HRFlow')">
    ...
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-2 text-sm leading-6 text-slate-600">
                Desde aquí puedes fichar, consultar tu calendario y gestionar tus solicitudes personales dentro del
                portal de tu empresa.
            </p>
            @if ($portalUser->canAccessAdministration())
                <p class="mt-1 text-sm leading-6 text-slate-500">
                    Tu perfil también puede entrar en la zona administrativa. Encontrarás el acceso directo en la
                    cabecera.
                </p>
            @endif
        </div>
        <a href="{{ route('portal.time-tracking.index', $tenantRouteParameters) }}"
            class="inline-flex items-center gap-1.5 self-start rounded-xl bg-amber-500 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-amber-400 sm:self-auto">
            Control horario
        ...
        <article class="rounded-2xl bg-slate-900 p-6 text-white">
            ...
            <div class="mt-4 flex items-center gap-4">
                @if ($portalUser->getFilamentAvatarUrl())
                    <img src="{{ $portalUser->getFilamentAvatarUrl() }}" alt="{{ $portalUser->name }}"
                        class="h-14 w-14 rounded-2xl object-cover ring-2 ring-white/20">
                @else
                    <div
                        class="flex h-14 w-14 items-center justify-center rounded-2xl bg-white/10 text-lg font-bold text-white">
            ...
            <div class="mt-6 grid grid-cols-3 gap-3">
                ...
                <div class="rounded-xl bg-white/8 p-3">
                    <p class="text-xs font-medium text-slate-400">Código</p>
                ...
                <div class="rounded-xl bg-white/8 p-3">
                    ...
                    <p class="mt-1.5 text-sm font-semibold text-white">{{ $portalUser->annual_vacation_days }} días
                        totales</p>
                    <p class="mt-1 text-xs text-slate-300">{{ $usedVacationDays }} consumidos ·
                        {{ $remainingVacationDays }} disponibles</p>
                </div>
            </div>
        </article>

        <article
            class="flex min-w-0 flex-col justify-between rounded-2xl bg-white p-6 ring-1 ring-slate-200 shadow-sm lg:min-w-64">
            <div>
            ...
            <div class="mt-4">
                @if ($todayOffReason && !$todayTimeEntry)
                    <span
                        class="inline-flex rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700 ring-1 ring-emerald-200">
                        ¡Disfruta del día!
                    </span>
        ...
        <article class="rounded-2xl bg-white p-5 ring-1 ring-slate-200 shadow-sm">
            <div class="flex items-center justify-between">
                ...
                @if ($pendingLeaveRequestsCount > 0)
                    <span
                        class="rounded-full bg-amber-50 px-2.5 py-1 text-xs font-semibold text-amber-700 ring-1 ring-amber-200">
                        {{ $pendingLeaveRequestsCount }} pend.
                    </span>
                @endif
            </div>
            <p class="mt-3 text-3xl font-semibold tabular-nums text-slate-900">{{ $totalLeaveRequestsCount }}</p>
            <p class="mt-1 text-xs text-slate-500">{{ $totalLeaveRequestsCount }}
                registrada{{ $totalLeaveRequestsCount !== 1 ? 's' : '' }}</p>
        </article>

        <article class="rounded-2xl bg-white p-5 ring-1 ring-slate-200 shadow-sm">
            <div class="flex items-center justify-between">
                <p class="text-xs font-semibold uppercase tracking-widest text-slate-400">Próxima ausencia</p>
                @if ($nextApprovedLeaveRequest)
                    <span
                        class="rounded-full bg-blue-50 px-2.5 py-1 text-xs font-semibold text-blue-700 ring-1 ring-blue-200">Aprobada</span>
                @endif
            </div>
    ...
    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <x-portal.nav-card eyebrow="Acceso directo" title="Calendario" :description="$calendarDescription" :href="route('portal.calendar.index', $tenantRouteParameters)" />
        <x-portal.nav-card eyebrow="Acceso directo" title="Control horario" :description="$timeTrackingDescription" :href="route('portal.time-tracking.index', $tenantRouteParameters)" />
        <x-portal.nav-card eyebrow="Acceso directo" title="Solicitudes" :description="$requestsDescription" :href="route('portal.requests.index', $tenantRouteParameters)" />
        <x-portal.nav-card eyebrow="Acceso directo" title="Documentación" :description="$documentDescription" :href="route('portal.documents.index', $tenantRouteParameters)" />
    </div>
</x-layouts.portal>
